...
func _process(delta):
	...
	if get_click>0: #any mouse button pressed 
		if not mouse_previously_clicked: #
			#var board_place: Vector3=get_second_global_position() #used just for debugging to know what triangle is being considered.
	...
	if recheck_movement_options: #
		...
		if thing_autoloaded.any_piece_selected and to_check.has_method("can_move_to"):
			...
			if ($center_triangle.global_position-to_check.get_second_global_position()).length()>sqrt(3)/3:
				able_to_be_chosen=to_check.can_move_to($center_triangle.global_position, true) #allow self-capture in order to run the code faster.
			...
			for piece in player_pieces:
				if (piece.get_second_global_position()-$center_triangle.global_position).length()<sqrt(3)/3:
					piece_on_spot=true #
					which_piece=piece
					break #
			...
			if not piece_on_spot:
				...
				for piece in player_pieces:
					if piece.has_method("can_move_to") and piece.can_move_to($center_triangle.global_position, true): #allow self-capture to run the code faster.
	...
	if just_clicked and able_to_be_chosen and mouse_inside and (not thing_autoloaded.any_move_to_position):
		...
		#print("triangle at "+str(get_second_global_position())+" was clicked and mouse is inside.")
	...
	if exists_a_piece_to_move_here or able_to_be_chosen or piece_on_spot:
		...
		$center_triangle/MeshInstance3D.visible=thing_autoloaded.fog_of_war #true #this blocks the piece from being seen.
	pass
